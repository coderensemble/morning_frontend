...
import {unhideArticle} from "../reducers/hiddenArticles";

function Header() {
  const dispatch = useDispatch();
  const user = useSelector((state) => state.user.value);
  const unhidenArticle = useSelector((state) => state.hiddenArticle);
  ...
  const handleRegister = () => {
    fetch("https://morning-backend.vercel.app/users/signup", {
      method: "POST",
  ...
  const handleConnection = () => {
    fetch("https://morning-backend.vercel.app/users/signin", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    if (isModalVisible) {
      userSection = (
        <div className={styles.headerIcons}>
          <FontAwesomeIcon onClick={() => handleEyeClick()} icon={faEye} className={styles.headerIcons} />
